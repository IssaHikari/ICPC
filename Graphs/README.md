...
<div align="center">
<img src="Screenshot 2026-06-03 183014.png" width="600px" alt="Problem Screenshot"/>
</div>
...
void dfs(int node, vector<long long> &Ci){
    visited[node] = true;
    Min_Nbr = min(Min_Nbr, Ci[node]);  // ✅ Include current node!
    for (auto child : adj[node]){
        if (!visited[child]){
            dfs(child, Ci);
        }
...
int main(){
    ...
    vector<long long> Ci(n + 1);       // ✅ 1-indexed
